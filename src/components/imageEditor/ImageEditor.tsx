...
const ImageEditor = (props: IImageEditorProps) => {
  ...
  const isTextActive = selectedObject && (selectedObject as any).isText;
  ...
  // Use text handlers hook
  const { handleAddText } = useTextHandlers(
    canvas,
    isCropping,
    isDrawing,
    currentColor,
  ...
  const handleSave = useCallback(() => {
    ...
    try {
      ...
      const objectsToExport = canvas.getObjects().filter((obj) => {
        ...
        // Include blur rects, shapes, drawings, and text
        const customObj = obj as any;
        return customObj.isBlurPatch || customObj.isDrawing || customObj.isText || (customObj.stroke && customObj.strokeWidth);
      });
