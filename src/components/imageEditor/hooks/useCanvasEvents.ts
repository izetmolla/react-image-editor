...
export const useCanvasEvents = (
  ...
  useEffect(() => {
    ...
    // Handle double-click on text objects to enter editing mode
    let clickTimeout: NodeJS.Timeout | null = null;
    let lastClickedText: fabric.Textbox | null = null;
    const handleMouseDown = (e: fabric.IEvent<Event>) => {
      const obj = e.target;
      if (obj && (obj as any).isText) {
        const textbox = obj as fabric.Textbox;
        if (clickTimeout && lastClickedText === textbox) {
        ...
        // Clicked on something else, clear timeout
        if (clickTimeout) {
          clearTimeout(clickTimeout);
          clickTimeout = null;
        }
